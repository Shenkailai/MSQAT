...
class SSASTModel(nn.Module):
    def __init__(self, label_dim=1,
        ...
        # use a pretrained models for finetuning
            ...
            print('now load a SSL pretrained models from ' +
    ...
    def mpc(self, x, mask_patch, cluster, show_mask=False):
        ...
        encode_samples = torch.empty(
            (B, mask_patch, 256), device=x.device, requires_grad=False).float()
        # size 12(batch_size) * 100(#mask_patch), index of masked patches
        mask_index = torch.empty(
            (B, mask_patch), device=x.device, requires_grad=False).long()
        ...
        for i in np.arange(0, B):
            ...
            total = torch.mm(encode_samples[i], torch.transpose(
                pred[i], 0, 1))  # e.g. size 100*100
        ...
        if show_mask == False:
            ...
            for i in range(B):
                result = [float(
                    t) * 99 for t in torch.eq(torch.argmax(self.softmax(total), dim=0), self.mask_correct)]
                pred[i, mask_index[i], :] = torch.tensor(result).reshape(
                    mask_patch, 1).expand(mask_patch, 256)
    ...
    def mpg(self, input, mask_patch, cluster):
        ...
        pred = torch.empty((B, mask_patch, self.fshape * self.tshape),
...
if __name__ == '__main__':
    ...
    ast_mdl = SSASTModel(
        ...
        pretrain_stage=True, task='pretrain_mpc').cuda()
    ...
    test_input = torch.zeros([1, input_tdim, 128]).cuda()
    ...
    ast_mdl = SSASTModel(label_dim=35,
                         fshape=16, tshape=16, fstride=10, tstride=10,
                         input_fdim=128, input_tdim=input_tdim, model_size='base',
                         pretrain_stage=False, load_pretrained_mdl_path='./test_mdl.pth').cuda()
